...
class Hotels extends Component {
    constructor(props) {
        ...
        this.state = { language, array_latest_requests, cities: [], latest_request, hotels: [], isLoading: true };
    }
    ...
    componentDidMount() {
        ...
        if (latest_request) {
            axios.post(process.env.REACT_APP_API_URL + "/hotels", { request: latest_request }).then(response => this.setState({ hotels: response.data, isLoading: false })).catch(error => console.error(error));
        }
    }
    ...
    save_local_storage = (latest_request) => {
        ...
        this.setState({ array_latest_requests }, () => {
            axios.post(process.env.REACT_APP_API_URL + "/hotels", { request: latest_request }).then(response => this.setState({ hotels: response.data, isLoading: false })).catch(error => console.error(error));
        });
    }

    render() {
        const { language, cities, latest_request, hotels, isLoading } = this.state;
            ...
            <main>
                <div className={styles.container}>
                    ...
                    {isLoading ? (
                        ...
                        hotels.length > 0 && <ShowHotels language={language} hotels={hotels} />
                    )}
                </div>
            </main>
        );
